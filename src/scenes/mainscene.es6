const GAUGE_INIT_RECOVERY_VALUE = 18;
/**
 * MainScene
 */
phina.define("MainScene", {
  ...
  init: function() {
    ...
    // 描画用Map 初期化
    let map = (this.map = []);
    for (let col = 0; col < GRID_NUM_X; col++) {
      this.map[col] = [];
      for (let row = 0; row < GRID_NUM_Y; row++) {
        // 上部を除く周囲にカベ
    ...
    // this.gauge.on('pointingStart', ()=>{this.specialFlush()});
    this.recoveryValue = GAUGE_INIT_RECOVERY_VALUE;
  ...
  update: function(app) {
    ...
    // 時間とともにゲージ回復量が減る
    if (this.timer % 16 === 0)
      this.recoveryValue = Math.max(5, this.recoveryValue - 1);
    ...
    if (
      kb.getKeyDown("right") ||
      (p.getPointingStart() && this.rightKey.hitTest(p.x, p.y))
    )
      rightKeyHandler();
    ...
    if (kb.getKeyDown("left")) {
      ...
      if (sideFlg === true) this.px--;
    }

    /* 下ボタン：入力による落下と自然落下とで同時に発生させない */
    ...
    if (kb.getKeyDown("z") || kb.getKeyDown("x")) {
      temp = kb.getKey("z")
        ? this.createRotatedPiece(this.piece, true)
        : this.createRotatedPiece(this.piece, false);
  ...
  stackCheck: function() {
    ...
    if (this.moveFlg === true) {
      ...
      if (this.gameoverFlg) {
        ...
        setTimeout(() => {
          this.exit({ score: this.score, level: this.currentLevel });
      ...
      this.gauge.value += this.recoveryValue;
  ...
  checkConnection: function() {
    /* 自分の周りをチェック：再帰処理 */
    const checkPeriphery = function(type, col, row, combo = false) {
  ...
  levelUpCheck: function() {
    if (
      this.score > BORDER_SCORE_UNIT * this.currentLevel &&
